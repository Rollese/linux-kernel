...
: "${_incremental:=yes}"
# kbuild verbosity. Empty = terse; V=1 for full command lines when debugging a
# build failure; -s for near silence.
: "${_verbose:=}"
...
build() {
  cd $_srcname
  # $_verbose: '' = terse kbuild output (CC [M] foo.o), V=1 = full command lines,
  # -s = near silent (warnings and errors only).
  make LLVM=1 LOCALVERSION='' $_verbose -j$(nproc) all
  make LLVM=1 LOCALVERSION='' $_verbose -j$(nproc) -C tools/bpf/bpftool vmlinux.h feature-clang-bpf-co-re=1
}
